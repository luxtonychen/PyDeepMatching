...
def construct_pyramid(response_maps, upper_bound, kernel_size = 4):
    ...
    pyramid_layer = {'size': (h, w), 'response_maps':maps, 'weights': np.ones(h*w), 'kernel_size': kernel_size, 'step': 1, 'pool_idx': None}
...
def get_next_layer(layer):
    ...
    h, w, maps, weights = sparse_conv2((layer['size'][0], layer['size'][1], maps), aggregation, layer['weights'], step)
    
    return {'size': (h, w), 'response_maps':maps, 'weights': weights, 'kernel_size': kernel_size, 'step': step, 'pool_idx': pool_idx}
